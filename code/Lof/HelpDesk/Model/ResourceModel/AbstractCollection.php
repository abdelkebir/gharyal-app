...
abstract class AbstractCollection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    protected function performAfterLoad($tableName, $columnName)
    {
        ...
        if (count($items)) {
            ...
            $select = $connection->select()->from(['lofhelpdesk_entity_store' => $this->getTable($tableName)])
                ->where('lofhelpdesk_entity_store.' . $columnName . ' IN (?)', $items);
    ...
    protected function performAfterLoadDepartment($tableName, $columnName)
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $select = $connection->select()->from(['lofhelpdesk_permission_department' => $this->getTable($tableName)])
                ->where('lofhelpdesk_permission_department.' . $columnName . ' IN (?)', $items);
            $result = $connection->fetchAll($select);
            if ($result) {
                foreach ($this as $item) {
                    $entityId = $item->getData($columnName);
    ...
    protected function performAfterLoadCategory($tableName, $columnName)
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $select = $connection->select('category_id')->from(['lofhelpdesk_department_category' => $this->getTable($tableName)])
                ->where('lofhelpdesk_department_category.' . $columnName . ' IN (?)', $items);
            $result = $connection->fetchAll($select);

...
class Index extends \Magento\Framework\App\Action\Action
{
    ...
    {		
		...
		$orderId=$current_order->getEntityId();  // database pk id
		
		$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
		$order = $objectManager->create('\Magento\Sales\Model\OrderRepository')->get($orderId);
		$orderNumber=$order->getIncrementId();  // order number visible to customer
		...
		$BillStateCode = '';
		if(!is_null($billerRegionId))
		{
			$bill_region = $this->_regionFactory->create()->load($billerRegionId);
			$BillStateCode = $bill_region['code'];
		}
		else
		{
			$BillStateCode = $spbillingAddress->getRegion();
		}
		
		$ShipStateCode = '';
		if(!is_null($shipperRegionId))
		{
			$ship_region = $this->_regionFactory->create()->load($shipperRegionId);
			$ShipStateCode = $ship_region['code'];
		}
		else
		{
			$ShipStateCode = $spshippingAddress->getRegion();
		}
		...
		$custom_data = array (
			...
			'SHIPPING_DETAIL' => array (
				...
				'SHIPPING_COST' => $this->encryptData(number_format((float)$order->getShippingAmount(), 2, '.', ''),$enc_key)
			),
			'ORDER' => array (
				'DISCOUNT_ON_TOTAL' => $this->encryptData(abs(number_format((float)($order->getBaseDiscountAmount()), 2, '.', '')),$enc_key),
				'SUBTOTAL' => $this->encryptData(number_format((float)($order->getGrandTotal()), 2, '.', ''),$enc_key)
			),
			'ADDITIONAL_DATA' => array (
				'BILL_TO_FORENAME' => $this->encryptData($this->TrimString($this->clean_special_chars($spbillingAddress->getFirstname()),60),$enc_key),
				'BILL_TO_SURNAME' => $this->encryptData($this->TrimString($this->clean_special_chars($spbillingAddress->getLastname()),60),$enc_key),
				...
				'BILL_TO_ADDRESS_LINE' => $this->encryptData($this->TrimString($this->clean_special_chars($billing_street_address),50),$enc_key),
				'BILL_TO_ADDRESS_CITY' => $this->encryptData($this->TrimString($this->clean_special_chars($spbillingAddress->getCity()),50),$enc_key),
				'BILL_TO_ADDRESS_STATE' => $this->encryptData($this->TrimString($this->clean_special_chars($BillStateCode),50),$enc_key),
				'BILL_TO_ADDRESS_COUNTRY' => $this->encryptData($this->TrimString($this->clean_special_chars($spbillingAddress->getCountryId()),50),$enc_key),
				'BILL_TO_ADDRESS_POSTAL_CODE' => $this->encryptData($order->getShippingAddress()->getPostcode(),$enc_key),
				
				'SHIP_TO_FORENAME' => $this->encryptData($this->TrimString($this->clean_special_chars($spshippingAddress->getFirstname()),60),$enc_key),
				'SHIP_TO_SURNAME' => $this->encryptData($this->TrimString($this->clean_special_chars($spshippingAddress->getLastname()),60),$enc_key),
				'SHIP_TO_PHONE' => $this->encryptData($spshippingAddress->getTelephone(),$enc_key),
				'SHIP_TO_ADDRESS_LINE' => $this->encryptData($this->TrimString($this->clean_special_chars($shipping_street_address),50),$enc_key),
				'SHIP_TO_ADDRESS_CITY' => $this->encryptData($this->TrimString($this->clean_special_chars($spshippingAddress->getCity()),50),$enc_key),
				'SHIP_TO_ADDRESS_STATE' => $this->encryptData($this->TrimString($this->clean_special_chars($ShipStateCode),50),$enc_key),
				'SHIP_TO_ADDRESS_COUNTRY' => $this->encryptData($this->TrimString($this->clean_special_chars($spshippingAddress->getCountryId()),50),$enc_key),
				...
				'REFERENCE_NUMBER' => $this->encryptData($orderNumber,$enc_key),
		...
		{
			$custom_data['ADDITIONAL_DATA']['MerchantFields']['MDD5'] = 'NO';
		...
		foreach ($order->getAllVisibleItems() as $item)
		{
			...
			$item_price=(float)$item_price > 0 ? $item_price:"1.00";			
			
			$OrderSummaryDescription[] = [
					'ITEM_NAME' => $this->encryptData($item->getName(),$enc_key),
					'QUANTITY' => $this->encryptData(intval($item->getQtyOrdered()),$enc_key),
					'UNIT_PRICE' => $this->encryptData($item_price,$enc_key),
					//'OLD_PRICE' => $this->encryptData($item->getSpecialPrice(),$enc_key),
					'CATEGORY' => $this->encryptData($this->TrimString($categoryArr[0]['CATEGORY'],99),$enc_key),
					'SUB_CATEGORY' => $this->encryptData($this->TrimString($categoryArr[0]['SUB_CATEGORY'],99),$enc_key)
					];
